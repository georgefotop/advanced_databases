...
joined_df = lapd.hint('SHUFFLE_HASH').join(filtered_df, lapd["PREC"] == filtered_df["AREA"])
...
joined_df = lapd.hint('SHUFFLE_HASH').join(filtered_df2, lapd["PREC"] == filtered_df["AREA"])
joined_df.explain()
# Keep specific columns in the result
...
joined_df = filtered_df.hint('SHUFFLE_HASH').crossJoin(lapd)
joined_df.explain()
...
joined_df2 = filtered_df2.hint('SHUFFLE_HASH').crossJoin(lapd)
joined_df2.explain()
