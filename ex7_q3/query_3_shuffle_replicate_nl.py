...
spark = SparkSession \
        .builder \
        .appName("Qeury 3-SHUFFLE_REPLICATE_NL") \
        .getOrCreate()
...
joined_df = rev.hint('SHUFFLE_REPLICATE_NL').join(df.select("Zip Code"), rev["ZIPcode"] == df["Zip Code"], "inner")
joined_df.explain()
...
filtered_df = crime_df.hint('SHUFFLE_REPLICATE_NL').join(unique_lat_lon_pairs, ["LAT", "LON"], "inner")
filtered_df.explain()
# Show the resulting DataFrame
#filtered_df.show()
