...
from pyspark.sql.functions import to_date, regexp_replace, to_timestamp, udf, expr, when, broadcast, year, month, desc, rank, col, when, count, collect_list
from pyspark.sql.window import Window


spark = SparkSession \
        .builder \
        .appName("Qeury 3-Broadcast") \
        .getOrCreate()
...
crime_df3 = crime_df.filter(col("LAT").isNotNull())
...
# Inner join of reverse_df and df on zip code
#joined_df = rev.hint('broadcast').join(df.select("Zip Code"), rev["ZIPcode"] == df["Zip Code"], "inner")
joined_df = rev.join(df.hint("broadcast").select("Zip Code"), rev["ZIPcode"] == df["Zip Code"], "inner")
joined_df.explain(extended = False)
...
joined_df= joined_df.drop("ZIPcode")


crime_df4 = crime_df.filter(col("LAT").isNotNull())
# Show the unique pairs of LAT and LON
#unique_lat_lon_pairs.show()

#############################
# Join df with unique_lat_lon_pairs on LAT and LON columns
filtered_df = crime_df.join(unique_lat_lon_pairs, ["LAT", "LON"], "inner")
