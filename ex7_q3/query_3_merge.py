...
joined_df = rev.hint('merge').join(df.select("Zip Code"), rev["ZIPcode"] == df["Zip Code"], "inner")
joined_df.explain()
...
filtered_df = crime_df.hint('merge').join((unique_lat_lon_pairs), ["LAT", "LON"], "inner")
filtered_df.explain()
# Show the resulting DataFrame
#filtered_df.show()
